...
/obj/structure/fireaxecabinet/unforgotten
	...
	icon = 'icons/obj/wallmounts.dmi'
	icon_state = "fireaxe"
	heirloom = /obj/item/weapon/greatsword/broadsword/psy/unforgotten/

/obj/structure/fireaxecabinet/unforgotten/Initialize()
	. = ..()
	heirloom = new /obj/item/weapon/greatsword/broadsword/psy/unforgotten
	desc = heirloom.desc
	update_icon()
...
/obj/structure/fireaxecabinet/unforgotten/attackby(obj/item/I, mob/user, params)
	if(open || obj_broken)
		if(istype(I, /obj/item/weapon/greatsword/broadsword/psy/unforgotten/) && !heirloom)
			var/obj/item/weapon/greatsword/broadsword/psy/unforgotten/F = I
			if(!user.transferItemToLoc(F, src))
				return
			heirloom = F
			to_chat(user, "<span class='notice'>I place the [F.name] back in the [name].</span>")
