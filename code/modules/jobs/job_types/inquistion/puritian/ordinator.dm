...
/datum/job/advclass/puritan/ordinator/after_spawn(mob/living/carbon/human/spawned, client/player_client)
	...
	switch(gear_choice)
		if("Covenant And Creed (Broadsword + Shield)")
			...
			spawned.clamped_adjust_skillrank(/datum/attribute/skill/combat/shields, 4, 4, TRUE)
			if(spawned.age == AGE_OLD)
		...
		if("Covenant and Consecratia (Flail + Shield)")
			...
			spawned.equip_to_slot_or_del(new /obj/item/weapon/shield/tower/metal/psy, ITEM_SLOT_BACK_R, TRUE)
			spawned.clamped_adjust_skillrank(/datum/attribute/skill/combat/whipsflails, 4, 4, TRUE)
			spawned.clamped_adjust_skillrank(/datum/attribute/skill/combat/shields, 4, 4, TRUE)
			if(spawned.age == AGE_OLD)
		...
		if("Crusade (Greatsword) and a Silver Dagger")
			...
			spawned.clamped_adjust_skillrank(/datum/attribute/skill/combat/swords, 4, 4, TRUE)
			spawned.clamped_adjust_skillrank(/datum/attribute/skill/combat/knives, 4, 4, TRUE)
			if(spawned.age == AGE_OLD)
				spawned.adjust_skillrank(/datum/attribute/skill/combat/swords, 1, TRUE)
