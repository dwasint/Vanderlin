/datum/job/advclass/mercenary/grenzelhoft
	...
	jobstats = list(STAT_CONSTITUTION = 2)
	skills = list(
...
/datum/job/advclass/mercenary/grenzelhoft/after_spawn(mob/living/carbon/human/H)
	...
	switch(weapon_choice)
		if("Zweihander")
			...
			H.adjust_stat_modifier(STATMOD_JOB, STAT_STRENGTH, 2) // They need this to roll at least min STR for the Zwei.
			H.adjust_skillrank(/datum/attribute/skill/combat/axesmaces, pick(2,3), TRUE) // Equal chance between skilled and average, can use a cudgel to beat less dangerous targets into submission
		...
		if("Musket")
			...
			H.adjust_skillrank(/datum/attribute/skill/combat/firearms, 3, TRUE)
		...
		if("Halberd")
			...
			H.equip_to_slot_or_del(new /obj/item/storage/belt/pouch/coins/poor, ITEM_SLOT_BELT_R, TRUE)
			H.equip_to_slot_or_del(new /obj/item/weapon/mace/cudgel, ITEM_SLOT_BELT_L, TRUE)
			H.adjust_stat_modifier(STATMOD_JOB, STAT_STRENGTH, 2) // same as zwei
			H.adjust_skillrank(/datum/attribute/skill/combat/axesmaces, pick(2,3), TRUE) // same as zwei
			H.adjust_skillrank(/datum/attribute/skill/combat/polearms, 3, TRUE)
