...
/datum/antagonist/aspirant/proc/give_equipment_prompt()
	...
	switch(chosen)
		...
		if(CHOICE_SKILL_SWORD)
			aspirant_mob.set_skillrank(/datum/attribute/skill/combat/swords, 6)

		if(CHOICE_SKILL_KNIFE)
			aspirant_mob.set_skillrank(/datum/attribute/skill/combat/knives, 6)

		if(CHOICE_SKILL_BOW)
			aspirant_mob.set_skillrank(/datum/attribute/skill/combat/bows, 6)

		if(CHOICE_SKILL_MACES)
			aspirant_mob.set_skillrank(/datum/attribute/skill/combat/axesmaces, 6)

		if(CHOICE_SKILL_LOCKPICKING)
			aspirant_mob.set_skillrank(/datum/attribute/skill/misc/lockpicking, 6)
		...
		if(CHOICE_GUN)
			...
			aspirant_mob.set_skillrank(/datum/attribute/skill/combat/firearms, 6)
			to_chat(owner, span_notice("I can retrieve my item from a statue, tree or clock by right clicking it."))

		if(CHOICE_BOMB)
			owner.special_items["Bomb"] = /obj/item/explosive/canister_bomb
			aspirant_mob.set_skillrank(/datum/attribute/skill/craft/bombs, 6)
			to_chat(owner, span_notice("I can retrieve my item from a statue, tree or clock by right clicking it."))
