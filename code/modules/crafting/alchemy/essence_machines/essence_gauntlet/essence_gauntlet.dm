...
/obj/item/clothing/gloves/essence_gauntlet/attack_right(mob/user)
	...
	for(var/i in 1 to stored_vials.len)
		...
		if(vial.contained_essence && vial.essence_amount > 0)
			...
			vial_desc += " - empty"
		vial_choices[vial_desc] = vial

	var/choice = input(user, "Which vial would you like to remove?", "Remove Vial") as null|anything in vial_choices
	if(!choice || !vial_choices[choice])
		return

	var/obj/item/essence_vial/chosen_vial = vial_choices[choice]
