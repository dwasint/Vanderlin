...
/client/proc/add_admin_verbs()
	if(holder)
		...
		add_verbs(GLOB.admin_verbs_default)
		if(rights & R_BUILD)
			add_verbs(/client/proc/togglebuildmodeself)
		if(rights & R_ADMIN)
			add_verbs(GLOB.admin_verbs_admin)
		if(rights & R_BAN)
			add_verbs(GLOB.admin_verbs_ban)
		if(rights & R_FUN)
			add_verbs(GLOB.admin_verbs_fun)
		if(rights & R_SERVER)
			add_verbs(GLOB.admin_verbs_server)
		if(rights & R_DEBUG)
			add_verbs(GLOB.admin_verbs_debug)
		if(rights & R_POSSESS)
			add_verbs(GLOB.admin_verbs_possess)
		if(rights & R_PERMISSIONS)
			add_verbs(GLOB.admin_verbs_permissions)
		if(rights & R_STEALTH)
			add_verbs(/client/proc/stealth)
		if(rights & R_ADMIN)
			add_verbs(GLOB.admin_verbs_poll)
		if(rights & R_SOUND)
			add_verbs(GLOB.admin_verbs_sounds)
			if(CONFIG_GET(string/invoke_youtubedl))
				add_verbs(/client/proc/play_web_sound)
		if(rights & R_SPAWN)
			add_verbs(GLOB.admin_verbs_spawn)
...
/client/proc/hide_most_verbs()//Allows you to keep some functionality while hiding some verbs
	...
	add_verbs(/client/proc/show_verbs)
...
/client/proc/show_verbs()
	...
	remove_verbs(/client/proc/show_verbs)
	add_admin_verbs()
