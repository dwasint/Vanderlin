...
/datum/chatOutput/proc/doneLoading()
	...
	sendClientData()
	syncRegex()
	SEND_TEXT(owner, "<span class=\"userdanger\">Failed to load fancy chat. Some features won't work.</span>") // do NOT convert to to_chat()
...
/proc/to_chat_immediate(target, message, handle_whitespace = TRUE, trailing_newline = TRUE)
	...
	if(islist(target))
		...
		if (!C)
			return
		SEND_TEXT(C, original_message) //Send it to the old style output window.
		if(!C.chatOutput || C.chatOutput.broken) // A player who hasn't updated his skin file.
			return
		if(!C.chatOutput.loaded)
