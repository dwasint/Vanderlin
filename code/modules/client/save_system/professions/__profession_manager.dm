...
/datum/profession_manager/proc/load_profession_data()
	...
	active_profession = save_manager.get_data("professions", "active", null)

/datum/profession_manager/proc/save_profession_data()
	save_manager.set_data("professions", "data", profession_data)
	save_manager.set_data("professions", "active", active_profession)
...
/datum/profession_manager/proc/handle_level_ups(profession_name, old_level, new_level, datum/profession/prof)
	...
	// If this is the active profession and we have a mob, refresh passives
	if(profession_name == active_profession && attached_mob)
		refresh_passives()
...
/datum/profession_manager/proc/set_active_profession(profession_name)
	if(!profession_name)
		active_profession = null
		save_profession_data()
		if(attached_mob)
			refresh_passives()
		return TRUE
	...
	initialize_profession(profession_name)
	active_profession = profession_name
	save_profession_data()
	if(attached_mob)
		refresh_passives()
	return TRUE

/datum/profession_manager/proc/get_active_profession_info()
	if(!active_profession)
		return null

	return get_profession_info(active_profession)

/datum/profession_manager/proc/has_recipe_unlocked(recipe_type, profession_name = null)
	if(!profession_name)
		profession_name = active_profession
	if(!profession_name)
		return FALSE

	var/list/prof_data = get_profession_info(profession_name)
...
/datum/profession_manager/proc/has_passive_unlocked(passive_type, profession_name = null)
	...
	var/list/prof_data = get_profession_info(profession_name)
	var/passive_type_text = "[passive_type]"
	return (passive_type_text in prof_data["unlocked_passives"])
...
/datum/profession_manager/proc/has_ability_unlocked(ability_type, profession_name = null)
	...
	var/list/prof_data = get_profession_info(profession_name)
	var/ability_type_text = "[ability_type]"
	return (ability_type_text in prof_data["unlocked_abilities"])
...
/datum/profession_manager/proc/get_unlocked_passives_of_type(passive_base_type, profession_name = null)
	...
	for(var/passive_text in prof_data["unlocked_passives"])
		var/passive_type = text2path(passive_text)
		if(passive_type && ispath(passive_type, passive_base_type))
			matching_passives += passive_type
...
/datum/profession_manager/proc/get_unlocked_abilities_of_type(ability_base_type, profession_name = null)
	...
	for(var/ability_text in prof_data["unlocked_abilities"])
		...
		if(ability_type && ispath(ability_type, ability_base_type))
			matching_abilities += ability_type
...
/datum/profession_manager/proc/apply_all_passives()
	if(!attached_mob || !active_profession)
		return

	var/list/passive_types = get_all_unlocked_passive_types(active_profession)
	for(var/passive_type in passive_types)
...
/proc/has_recipe_unlocked(ckey, recipe_type, profession_name = null)
	var/datum/profession_manager/PM = get_profession_manager(ckey)
	if(PM)
		return PM.has_recipe_unlocked(recipe_type, profession_name)
	return FALSE
...
/proc/get_unlocked_passives_of_type(ckey, passive_base_type, profession_name = null)
	var/datum/profession_manager/PM = get_profession_manager(ckey)
	if(PM)
		return PM.get_unlocked_passives_of_type(passive_base_type, profession_name)
	return list()
