...
/datum/component/chimeric_organ/proc/handle_node_injection(tier, purity, slot, datum/chimeric_organs/injected_node, overlay_state)
	SIGNAL_HANDLER
	...
	switch(slot)
		if(INPUT_NODE)
			var/datum/chimeric_organs/input/injected_input = injected_node
		...
		if(OUTPUT_NODE)
			var/datum/chimeric_organs/output/injected_output = injected_node
		...
		if(SPECIAL_NODE)
			var/datum/chimeric_organs/special/injected_special = injected_node
...
/datum/component/chimeric_organ/proc/handle_input_injection(datum/chimeric_organs/input/injected_input)
	if(!partnerless_outputs.len)
		partnerless_inputs += injected_input
		return

	var/datum/chimeric_organs/output/picked_output = find_compatible_output(injected_input)
	if(!picked_output)
		partnerless_inputs += injected_input
		return
...
/datum/component/chimeric_organ/proc/handle_output_injection(datum/chimeric_organs/output/injected_output)
	if(!partnerless_inputs.len)
		partnerless_outputs += injected_output
		return

	var/datum/chimeric_organs/input/picked_input = find_compatible_input(injected_output)
	if(!picked_input)
		partnerless_outputs += injected_output
		return
...
/datum/component/chimeric_organ/proc/find_compatible_output(datum/chimeric_organs/input/input_node)
	...
	for(var/datum/chimeric_organs/output/listed_output as anything in partnerless_outputs)
...
/datum/component/chimeric_organ/proc/find_compatible_input(datum/chimeric_organs/output/output_node)
	...
	for(var/datum/chimeric_organs/input/listed_input as anything in partnerless_inputs)
...
/datum/component/chimeric_organ/proc/pair_input_output(datum/chimeric_organs/input/input_node, datum/chimeric_organs/output/output_node)
...
/datum/component/chimeric_organ/proc/attach_special_to_input(datum/chimeric_organs/input/input_node)
	if(!attachment_less_special_inputs.len)
		return

	var/datum/chimeric_organs/special/listed_special = pick(attachment_less_special_inputs)
	listed_special.attached_input = input_node
	attachment_less_special_inputs -= listed_special

/datum/component/chimeric_organ/proc/attach_special_to_output(datum/chimeric_organs/output/output_node)
	if(!attachment_less_special_outputs.len)
		return

	var/datum/chimeric_organs/special/listed_special = pick(attachment_less_special_outputs)
	listed_special.attached_output = output_node
	attachment_less_special_outputs -= listed_special

/datum/component/chimeric_organ/proc/handle_special_injection(datum/chimeric_organs/special/injected_special)
	...
	switch(injected_special.attachement_type)
		if(INPUT_NODE)
			...
			if(!available_inputs.len)
				...
				return
			var/datum/chimeric_organs/input/picked_input = pick(available_inputs)
		...
		if(OUTPUT_NODE)
			...
			if(!available_outputs.len)
				...
				return
			var/datum/chimeric_organs/output/picked_output = pick(available_outputs)
