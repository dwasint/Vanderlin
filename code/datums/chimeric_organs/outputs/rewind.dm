/datum/chimeric_organs/output/rewinding
	name = "rewinding"
	desc = "When activated teleports you back to the last place you were hit."

	var/turf/last_hit_turf

/datum/chimeric_organs/output/rewinding/register_listeners(mob/living/carbon/target)
	if(!target)
		return
...
/datum/chimeric_organs/output/rewinding/trigger_effect(multiplier)
	. = ..()
	if(!last_hit_turf)
		return
	hosted_carbon.forceMove(last_hit_turf)

/datum/chimeric_organs/output/rewinding/proc/on_attacked(datum/source, datum/stress_event/event)
